...
<body>
	<%
		...
		List<OrderBean> adminOrderDetail = (List<OrderBean>) request.getAttribute("adminOrderDetail");
		OrderBean ob = adminOrderDetail.get(0);
		int total = 0;
	%>
	<h1>주문상세보기/수정</h1>
	...
	<table border="1">
		...
		<%
			for (int i = 0; i < adminOrderDetail.size(); i++) {
				OrderBean anOrder = adminOrderDetail.get(i);
		%>
		<tr>
			<td><%=anOrder.getO_g_name()%></td>
			<td><%=anOrder.getO_g_amount()%></td>
			<td><%=anOrder.getO_sum_money()%></td>
			<td><%=anOrder.getO_g_size()%></td>
			<td><%=anOrder.getO_g_color()%></td>
		</tr>
		<%
			total += anOrder.getO_sum_money();
			} // close for-loop
		%>
	</table>
	<h3>배송지정보</h3>
	받는사람:<%=ob.getO_receive_name()%><br> 집전화:<%=ob.getO_receive_phone()%><br>
	휴대폰:<%=ob.getO_receive_mobile()%><br> 배송지주소:<%=ob.getO_receive_addr1()%><br>
	배송지나머지주소:<%=ob.getO_receive_addr2()%><br> 기타요구사항:<%=ob.getO_memo()%><br>
	<h3>결제정보</h3>
	<form action="./AdminOrderModify.ao" method="post">
		<input type="hidden" name="trade_num" value="<%=ob.getO_trade_num()%>">
		주문합계금액:
		<%=total%><br> 결제방법:<%=ob.getO_trade_type()%><br> 입금자명:<%=ob.getO_trade_payer()%><br>
		...
		</select> <br> 운송장번호:<input type="text" name="trans_num"
			value="<%=ob.getO_trans_num()%>"><br> <input
