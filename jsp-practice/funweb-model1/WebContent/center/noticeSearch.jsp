...
<body>
	<div id="wrap">
		...
		<%
			...
			// 검색된 글의 개수 int count = getBoardCount(search)
			...
			<table id="notice">
				...
				<tr>
					...
					<td class="left">
						...
						%><img src="../images/center/level.gif" /><img
						src="../images/center/re.gif" /> <%
 	}
 %><a href="content.jsp?num=<%=bb.getNum()%>&pageNum=<%=pageNum%>&search=<%=search%>"><%=bb.getSubject()%></a>
					</td>
					<td><%=bb.getName()%></td>
					<td><%=bb.getDate()%></td>
			...
			<div id="page_control">
				...
				%><a href="noticeSearch.jsp?pageNum=<%=prevPage%>&search=<%=search%>">Prev</a>
				<%
					}
						// [1]~[10]
						for (int pageIdx = startPage; pageIdx <= endPage; pageIdx++) {
				%><a href="noticeSearch.jsp?pageNum=<%=pageIdx%>&search=<%=search%>">[<%=pageIdx%>]
				</a>
				<%
					}
						// [다음]
				...
				%><a href="noticeSearch.jsp?pageNum=<%=nextPage%>&search=<%=search%>">Next</a>
				<%
					}
					}
				%>
