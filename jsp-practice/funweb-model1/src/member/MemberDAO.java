...
public class MemberDAO {
	...
	public int checkIdDuplication(String id) {
		int isDuplicate = 1; // isDuplicate == 1 아이디 중복됨
		try {
			// 1단계 DB 연결
			con = getConnection();
			// 2단계 SQL 객체 생성 조건
	...
	public void insertMember(MemberBean jbean) {
		try {
			...
			con = getConnection();
			// 3단계 SQL 준비 객체 생성
			sql = "INSERT INTO member(id, passwd, name, reg_date, age, gender, email, address, phone, mobile) VALUES (?,?,?,?,?,?,?,?,?,?);";
			pstmt = con.prepareStatement(sql);
			...
			pstmt.setString(8, jbean.getAddress());
