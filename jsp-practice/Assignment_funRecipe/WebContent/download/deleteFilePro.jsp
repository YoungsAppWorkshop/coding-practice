...
<body>
	<%
		...
		// DB 작업 : 자료실 자료 삭제 메소드 호출
		DownloadDAO bdao = new DownloadDAO();
		boolean isSuccess = bdao.deleteFile(sId, num); 
		if (isSuccess) { // 삭제 완료한 경우 메시지 출력 후 fileList.jsp로 이동 
	%>
	<script>
			alert("자료를 삭제하였습니다.");
			location.href="fileList.jsp?pageNum=<%=pageNum%>";
	</script>
	<%
